...
export default createStore({
  ...
  actions: {
    ...
    async register({ dispatch }, creds) {
      console.log(creds);
      let response = await axios.post('https://mynots.gulfytrend.com/api/api/register', creds)
      return dispatch('attempt', response.data.token)
    },

    logout({commit}) {
      return axios.post('https://mynots.gulfytrend.com/api/api/logout').then(res => {
        commit('setToken', null)
        commit('setUser', null)
    ...
    getCategories({ commit }) {
      axios.get('https://mynots.gulfytrend.com/api/api/dashboard').then(res => {
    ...
    addCategory({commit}, NewCategory) {
      axios.post('https://mynots.gulfytrend.com/api/api/category/store', NewCategory)
      commit('addCategory', NewCategory)
    },

    deleteCat({ commit, dispatch }, category) {
      axios.delete('https://mynots.gulfytrend.com/api/api/category/' + category.id).then(res => {
          if(res.status == 200) {
    ...
    addNote({commit}, NewNote) {
      axios.post('https://mynots.gulfytrend.com/api/api/notes/store', NewNote).then(res => {
        if (res.status == 200) {
            commit('addNote')
        }
      })
    ...
    deleteNote({ commit }, note) {
      axios.delete('https://mynots.gulfytrend.com/api/api/notes/' + note.id).then(res => {
          if(res.status == 200) {
    ...
    updateNote({ commit }, data) {
      axios.put('https://mynots.gulfytrend.com/api/api/notes/' + data.id, data).then(res => {
          if(res.status == 200) {
